...
class PythonVenvInspector:
    ...
    def inspect(
        ...
        root = target.path.expanduser().resolve()
        if not root.is_dir() or root.is_symlink():
            raise InspectionFailed(
                backend=self.name,
                operation="inspect",
                message=f"Python environment is missing or invalid: {root}",
            )
        ...
        if not interpreter.is_file():
            raise InspectionFailed(
                backend=self.name,
                operation="inspect",
                message=f"Python virtual environment has no interpreter: {interpreter}",
            )
        settings = uv_settings(context, error=_inspection_error)
        ...
        try:
            ...
            raise _as_inspection_failed(exc) from exc
        if not runtime.is_virtual_environment:
            raise InspectionFailed(
                backend=self.name,
                operation="inspect",
                message=f"Target is not an isolated virtual environment: {root}",
            )
        try:
            result = runner.run(
                Command(
        ...
        packages, diagnostics = _normalize_distributions(raw, root, runtime.target_id)
        return EnvironmentState(
            ...
            environment=EnvironmentTarget(root, "python-venv"),
        )
...
def _normalize_distributions(
    ...
    if not isinstance(value, list):
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message="Python distribution metadata must be a list",
        )
    ...
    for index, item in enumerate(value):
        if not isinstance(item, dict):
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Python distribution metadata item {index} must be an object",
            )
        name = _string(item.get("name"), f"distribution {index} name")
        version = _string(item.get("version"), f"distribution {name} version")
        normalized = name.lower().replace("_", "-")
        if normalized in seen:
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Python environment contains duplicate distribution {normalized}",
            )
        seen.add(normalized)
        metadata_path = Path(_string(item.get("path"), f"distribution {name} path")).resolve()
        if not metadata_path.is_relative_to(root):
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Distribution {name} is loaded from outside the virtual environment",
            )
        ...
        if not isinstance(record_errors, list) or not all(
            ...
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Distribution {name} returned invalid RECORD verification data",
            )
        if record_errors:
            raise InspectionFailed(
                backend="python-venv",
        ...
        source, artifact, checksum, editable = _direct_identity(item.get("direct_url"), name)
        if editable:
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Editable distribution {name} is not supported in exact environments",
            )
        installer = item.get("installer")
        if source is None:
            source = (
                f"installed:{installer}"
        ...
        dependencies = _requires_dist(item.get("requires", []), name)
        packages.append(
            ResolvedPackage(
                ecosystem="pypi",
                name=name,
...
def _direct_identity(
    ...
    if not isinstance(value, dict):
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message=f"Distribution {package} direct_url.json must be an object",
        )
    if "_error" in value:
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message=f"Distribution {package} has unreadable direct_url.json: {value['_error']}",
        )
    url = value.get("url")
    if not isinstance(url, str) or not url:
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message=f"Distribution {package} direct_url.json has no URL",
        )
    parsed = urlsplit(url)
    if parsed.username is not None or parsed.password is not None or parsed.query:
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message=f"Distribution {package} direct URL contains credentials or query parameters",
        )
...
def _requires_dist(value: object, package: str) -> tuple[PackageReference, ...]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        raise InspectionFailed(
            backend="python-venv",
            operation="inspect",
            message=f"Distribution {package} requirements must be a list of strings",
        )
    ...
    for text in value:
        try:
            ...
            raise InspectionFailed(
                backend="python-venv",
                operation="inspect",
                message=f"Distribution {package} has invalid Requires-Dist {text!r}: {exc}",
            ) from exc
        dependencies.append(
            PackageReference(
                ecosystem="pypi",
...
def _inspection_error(message: str) -> InspectionFailed:
    return InspectionFailed(backend="python-venv", operation="inspect", message=message)


def _as_inspection_failed(error: BackendError) -> InspectionFailed:
